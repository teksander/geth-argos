...
class Logger(object):
    ...
    """
    def __init__(self, logfile, header, rate = 0, buffering = 1, ID = None, extra = False):
        """_summary_
        ...
        Args:
            ...
            buffering (int, optional): _description_. Defaults to 1.
            ID (_type_, optional): _description_. Defaults to None.
            extra (bool, optional): if you want more info to be added at each line logged, such as time and id. Defaults to False.
        """
        ...
        if extra:
            self.file.write(f'Time,ID,{pHeader}\n')
        else:
            self.file.write(f'{pHeader}\n')

        self.id = ID or open("/boot/pi-puck_id", "r").read().strip()
        self.close()
    ...
    def log(self, data):
        ...
        #             self.file.write(f'{self.id},{tString},{pData}\n')
    ...
    def change_file(self, filename):
        ...
        pHeader = ','.join([str(x) for x in self.header])
        with open(self.logfile, 'w+', buffering = self.buffering*len(self.header)) as f:
            f.write(f'{pHeader}\n')
