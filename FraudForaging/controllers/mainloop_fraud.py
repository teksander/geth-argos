    ...
    def query(self, min_confirmations=0):
        ...
        if not self.tx:
            ...
            if self.last < confirmations:
                ...
                print('Confirming: %s/%s', confirmations, min_confirmations)
...
def init():
    ...
    nav = Navigate(robot, navSpeed, withKF = True, fric=friction*int(robotID))

    # /* Init Random-Walk, __walking process */
    robot.log.info('Initialising random-walk...')
    rw = RandomWalk(robot, rwSpeed, fric=friction*int(robotID))
...
def controlstep():
    ...
    if not startFlag:
        ...
        def going(target):
            ...
            if nav.get_distance_to((target[0], target[1]), use_kf_obs=True) < params['source']['radius']/3:
                nav.avoid(move=True)
            else:
        ...
        if fsm.query(Idle.IDLE):
            ...
            if robot.variables.get_attribute("at")=='source':
                ...
                source_list = w3.sc.functions.getSourceList().call()
                ticketPrice = depoValueEst(source_list, myCeratinty)
                transactHash = w3.sc.functions.reportNewPt(int(sum(avgx)/len(avgx) * DECIMAL_FACTOR),
                                                           int(sum(avgy)/len(avgy) * DECIMAL_FACTOR), 1,
                                                           w3.toWei(ticketPrice, 'ether'),
                                                           int(myCeratinty * DECIMAL_FACTOR)).transact(
                    {'from': me.key, 'value': w3.toWei(ticketPrice, 'ether'), 'gas': gasLimit, 'gasPrice': gasprice})
                print('Robot ', robot.variables.get_id(), ' report source point: ', int(pos_state[0][0] * DECIMAL_FACTOR),
