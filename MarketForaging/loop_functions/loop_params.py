...
params['market']['radius']    =  math.sqrt((0.03*float(params['environ']["ARENADIMX"])*float(params['environ']["ARENADIMY"]))/math.pi)

# Parameters for dropzone
params['dropzone'] = dict()
params['dropzone']['position'] = [0, 0]
params['dropzone']['radius']   = math.sqrt((0.09*float(params['environ']["ARENADIMX"])*float(params['environ']["ARENADIMY"]))/math.pi)
...
params['patches']['dist_min']  = 1.5  * params['dropzone']['radius']
